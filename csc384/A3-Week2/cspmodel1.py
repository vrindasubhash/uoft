...
def kropki_model(board):
    ...
    # satisfying tuples for each constraint type
    ...
    csp.add_constraint(create_row_and_col_constraints(dim, diff_tuples, variables))
    
    # add cage constraints
    csp.add_constraint(create_cage_constraints(dim, diff_tuples, variables))
    
    # add dot constraints (white and black)
    csp.add_constraint(create_dot_constraints(dim, board.dots, white_tuples, black_tuples, variables))
    
    # add no dot constraints
    csp.add_constraint(create_no_dot_constraints(dim, board.dots, no_dot_tuples, variables))
...
def create_variables(dim, board):
    ...
    for row in range(dim):
        for col in range(dim):
            ...
            var = Variable(f"Var({row}, {col})", domain)
            variables.append(var)  # Flattening into a single list
    return variables
