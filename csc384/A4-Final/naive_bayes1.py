...
def explore(bayes_net, question):
    ...
    ''' 
    # helper to compute probability of salary >= $50K
    def compute_probability(bayes_net, evidence, gender=None):
        query_var = Variable("Salary", salary_variable_domains["Salary"])
        evidence_vars = [
            Variable(key, salary_variable_domains[key]) for key in evidence.keys()
        ]
        if gender:
            evidence["Gender"] = gender
        evidence_vars_with_values = {var: evidence[var.name] for var in evidence_vars}
        result = ve(bayes_net, query_var, evidence_vars_with_values)
        return result.values[query_var.value_index('>=50K')]

    data_file = "adult-test.csv" 
    evidence_set = []
    with open(data_file, newline='', encoding='utf-8-sig') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            evidence_set.append(row)
   
    # initialize counters
    total_women, total_men = 0, 0
    women_predicted_high, men_predicted_high = 0, 0
    women_correct, men_correct = 0, 0
    women_better_without_gender, men_better_without_gender = 0, 0

    for evidence in evidence_set:
        gender = evidence["Gender"]
        actual_salary = evidence["Salary"]

        # compute base probability without gender
        base_prob = compute_probability(bayes_net, evidence)

        # compute probability with gender
        gender_prob = compute_probability(bayes_net, evidence, gender)

        # update counters
        if gender == "Female":
            total_women += 1
            if base_prob > 0.5:
                women_predicted_high += 1
                if actual_salary == ">=50K":
                    women_correct += 1
            if base_prob > gender_prob:
                women_better_without_gender += 1
        elif gender == "Male":
            total_men += 1
            if base_prob > 0.5:
                men_predicted_high += 1
                if actual_salary == ">=50K":
                    men_correct += 1
            if base_prob > gender_prob:
                men_better_without_gender += 1

    # return based on the question
    if question == 1:
        return (women_predicted_high / total_women) * 100
    elif question == 2:
        return (men_predicted_high / total_men) * 100
    elif question == 3:
        return (women_better_without_gender / total_women) * 100
    elif question == 4:
        return (men_better_without_gender / total_men) * 100
    elif question == 5:
        return (women_correct / women_predicted_high) * 100 if women_predicted_high > 0 else 0
    elif question == 6:
        return (men_correct / men_predicted_high) * 100 if men_predicted_high > 0 else 0
    else:
