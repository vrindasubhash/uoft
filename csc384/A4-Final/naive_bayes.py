...
def explore(bayes_net, question):
    ...
    total_women = 0
    total_men = 0
    predicted_women_50K = 0
    predicted_men_50K = 0
    condition_women_count = 0
    condition_men_count = 0
    true_positive_women = 0
    true_positive_men = 0

    # get variables by their names from the Bayesian network
    ...
    salary_var = get_variable_by_name('Salary')
    gender_var = get_variable_by_name('Gender')

    # iterate through test dataset
    for row in test_data:
        # data for each individual
        evidence = {get_variable_by_name(header): value for header, value in zip(headers, row) if header != 'Salary'}

        # set evidence in Bayesian network
        ...
        if question == 1 or question == 2:
            # calculate P(Salary >= 50K | Evidence)
            result_factor = ve(bayes_net, salary_var, list(evidence.keys()))
            probability_50K = result_factor.get_value(['>=50K'])

            # gender specific checks
            if row[headers.index('Gender')] == 'Female':
                total_women += 1
                if probability_50K > 0.5:
                    predicted_women_50K += 1
            elif row[headers.index('Gender')] == 'Male':
                total_men += 1
                if probability_50K > 0.5:
                    predicted_men_50K += 1
            ...
            # calculate P(Salary >= 50K | Evidence)
            result_factor_no_gender = ve(bayes_net, salary_var, [var for var in evidence.keys() if var != gender_var])
            probability_no_gender = result_factor_no_gender.get_value(['>=50K'])

            result_factor_with_gender = ve(bayes_net, salary_var, list(evidence.keys()))
            probability_with_gender = result_factor_with_gender.get_value(['>=50K'])

            # gender specific checks
            if row[headers.index('Gender')] == 'Female':
                if probability_no_gender > probability_with_gender:
                    condition_women_count += 1
            elif row[headers.index('Gender')] == 'Male':
                if probability_no_gender > probability_with_gender:
                    condition_men_count += 1
            ...
            # check if model prediction was >= 50K
            if probability_50K > 0.5:
                actual_salary = row[headers.index('Salary')]
                if actual_salary == '>=50K':
                    # True positive case
                    if row[headers.index('Gender')] == 'Female':
                        true_positive_women += 1
                    elif row[headers.index('Gender')] == 'Male':
                        true_positive_men += 1

    # calculate result for each question
    if question == 1:
        return (predicted_women_50K / total_women) * 100 if total_women > 0 else 0
    elif question == 2:
        return (predicted_men_50K / total_men) * 100 if total_men > 0 else 0
    elif question == 3:
        return (condition_women_count / total_women) * 100 if total_women > 0 else 0
    elif question == 4:
        return (condition_men_count / total_men) * 100 if total_men > 0 else 0
    elif question == 5:
        return (true_positive_women / predicted_women_50K) * 100 if predicted_women_50K > 0 else 0
    elif question == 6:
        return (true_positive_men / predicted_men_50K) * 100 if predicted_men_50K > 0 else 0
    else:
