...
def heuristic_advanced(board):
    ...
    :rtype: int
    """
    # idea is to sum the manhatten distance with a very large extra penalty if a box would be stuck in a corner 
    # (because that move wouldn't be successful as the box would be stuck so you can prune it) 
    manhattan_distance = heuristic_basic(board)
    stuck_penalty = 0
    LARGE_COST = 1000 # large cost for being in a corner
    
    for box in board.boxes:
        # check if the box is in a corner and not on a storage spot 
        if (box[0], box[1]) not in board.storage:
            # check if the box is stuck in a corner (two adjacent obstacles)
            if ((box[0] + 1, box[1]) in board.obstacles and (box[0], box[1] + 1) in board.obstacles) or \
               ((box[0] + 1, box[1]) in board.obstacles and (box[0], box[1] - 1) in board.obstacles) or \
               ((box[0] - 1, box[1]) in board.obstacles and (box[0], box[1] + 1) in board.obstacles) or \
               ((box[0] - 1, box[1]) in board.obstacles and (box[0], box[1] - 1) in board.obstacles):
                stuck_penalty += LARGE_COST # large penalty cost so you avoid this state

    # add the stuck penalty to the manhattan distance
    return manhattan_distance + stuck_penalty
