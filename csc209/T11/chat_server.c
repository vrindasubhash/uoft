...
int read_from(int client_index, struct sockname *users) {
    ...
    if (users[client_index].username == NULL) {
        // We have not received the username yet, so copy buf to username
        users[client_index].username = strdup(buf);
        printf("Received username: %s\n", users[client_index].username);
    } else {
        // We have received the username, so this is a message to broadcast
        printf("Broadcasting message from %s: %s\n", users[client_index].username, buf);
        for (int i = 0; i < MAX_CONNECTIONS; i++) {
            if (users[i].sock_fd != -1 && users[i].sock_fd != fd) {
