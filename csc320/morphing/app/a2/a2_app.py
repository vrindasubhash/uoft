...
class Assignment2App(base_app.BaseAssignmentApp):
    ...
    def draw_image(self):
        ...
        if not self.args.destination_image_path:
            ...
            self.destination_image = np.flip(
                img_ops.read_image(self.args.destination_image_path), 0
            )
            if (
                self.destination_image.shape[0] != self.source_image.shape[0]
                or self.destination_image.shape[1] != self.destination_image.shape[1]
            ):
                print(
                    "The left image dimensions should match the right image dimensions."
        ...
        if input_ratio < screen_ratio:
            ...
            self.rescaled_height = int(
                self.source_image.shape[0] * (1.0 / self.rescale_ratio)
            )
            ...
            self.rescaled_width = int(
                self.source_image.shape[1] * (1.0 / self.rescale_ratio)
            )
        ...
        # S
        self.rescaled_source = cv2.resize(
            self.source_image,
            (self.rescaled_width, self.rescaled_height),
            interpolation=cv2.INTER_AREA,
        )
        # D
        self.rescaled_destination = cv2.resize(
    ...
    def update_buffer(self):
        ...
        if self.left_buffer == "S":
            self.buffer[self.y_min : self.y_max, self.x_min : self.x_max] = (
                self.rescaled_source
            )
        elif self.left_buffer == "WD":
            self.buffer[self.y_min : self.y_max, self.x_min : self.x_max] = (
                self.rescaled_warped_destination
            )
        elif self.left_buffer == "M":
            self.buffer[self.y_min : self.y_max, self.x_min : self.x_max] = (
                self.rescaled_morphed_image
            )

        if self.right_buffer == "D":
            self.buffer[
                self.y_min : self.y_max, (w // 2) + self.x_min : (w // 2) + self.x_max
            ] = self.rescaled_destination
        elif self.right_buffer == "WS":
            self.buffer[
                self.y_min : self.y_max, (w // 2) + self.x_min : (w // 2) + self.x_max
            ] = self.rescaled_warped_source
        elif self.right_buffer == "M":
            self.buffer[
                self.y_min : self.y_max, (w // 2) + self.x_min : (w // 2) + self.x_max
            ] = self.rescaled_morphed_image

    def check_image_bounds(self, coord: npt.NDArray[np.float32]) -> list | bool:
        ...
        if (
            coord[0] < self.x_max
            and coord[0] > self.x_min
            and coord[1] < self.y_max
            and coord[1] > self.y_min
        ):
            return True, "left"
        elif (
            coord[0] < self.x_max + (w // 2)
            and coord[0] > self.x_min + (w // 2)
            and coord[1] < self.y_max
            and coord[1] > self.y_min
        ):
            return True, "right"
    ...
    def canvas_to_image(
        self, coords: npt.NDArray[np.float32], side: str
        ...
        Args:
            ...
            side (str): string containing either "left" or "right" to specify which side of the screen.
        Returns:
        ...
        if side == "left":
            image_coords = np.concatenate(
                [coords[..., 0:1] - self.x_min, coords[..., 1:2] - self.y_min], axis=-1
            )
        ...
        if side == "left":
            ...
            canvas_coords = np.concatenate(
                [
                    ...
                    canvas_coords[..., 1:2] + self.y_min,
                ],
                axis=-1,
            )
            return canvas_coords.astype(int)
        elif side == "right":
            canvas_coords = coords.astype(np.float32) / self.rescale_ratio
            canvas_coords = np.concatenate(
                [
    ...
    def init_state(self, first: Optional[bool] = True):
        ...
        if first and self.args.source_line_path:
            self.source_morph_lines = csv_ops.load_csv(
                self.args.source_line_path
            ).reshape(-1, 2, 2)
            self.source_morph_lines = img_ops.normalize_coordinates(
                self.image_to_canvas(self.source_morph_lines, "left"),
                *self.app_resolution,
            )
            self.source_morph_index = (
                self.source_morph_lines.shape[0] * self.source_morph_lines.shape[1]
            )
            ...
            if not self.args.destination_line_path:
                ...
                self.destination_morph_lines = csv_ops.load_csv(
                    self.args.destination_line_path
                ).reshape(-1, 2, 2)
                self.destination_morph_lines = img_ops.normalize_coordinates(
                    self.image_to_canvas(self.destination_morph_lines, "right"),
                    *self.app_resolution,
                )
                self.destination_morph_index = (
                    self.destination_morph_lines.shape[0]
                    * self.destination_morph_lines.shape[1]
                )
    ...
    def get_arrow_heads(
        ...
        head_length: Optional[np.float32] = 2e-2,
    ) -> npt.NDArray[np.float32] | npt.NDArray[np.float32]:
        """Calculate location of arrowheads"""
        U = (destination - origin) / (1e-4 + np.linalg.norm(destination - origin))
        V = np.array([-U[1], U[0]])
        v1 = destination - head_length * U + head_length * V
        v2 = destination - head_length * U - head_length * V

        return v1, v2

    def add_raster_arrow(self, origin, destination, color, line_width):
        """Small wrapper on top of add_raster_line that draws an arrow."""
        head_0, head_1 = self.get_arrow_heads(origin, destination)
        ...
        if morph_index % 2 == 1:
            if right and self.focus_pane == "left":
                self.add_raster_arrow(
                    ...
                    palette[num_full_lines % num_colors],
                    line_width=4,
                )
            elif right and self.focus_pane == "right":
                self.add_raster_arrow(
                    morph_lines[-1, 0],
                    np.array([self.mouse_location[0], self.mouse_location[1]]),
                    palette[num_full_lines % num_colors],
                    line_width=4,
                )
    ...
    def render(self):
        ...
        if self.dirty_canvas or self.run_realtime:
            ...
            if num_lines > 0:
                source_morph_lines = img_ops.unnormalize_coordinates(
                    self.source_morph_lines[:num_lines], h, w
                )
                destination_morph_lines = img_ops.unnormalize_coordinates(
                    self.destination_morph_lines[:num_lines], h, w
                )
                source_morph_lines = img_ops.normalize_coordinates(
                    self.canvas_to_image(source_morph_lines, "left"),
                    *self.source_image.shape[:2],
                )
                destination_morph_lines = img_ops.normalize_coordinates(
                    self.canvas_to_image(destination_morph_lines, "right"),
                    *self.source_image.shape[:2],
                )

                self.warped_source = algos.run_a2_algo(
                    self.source_image,
                    self.destination_image,
                    source_morph_lines,
                ...
                if self.args.destination_image_path:
                    self.warped_destination = algos.run_a2_algo(
                        self.destination_image,
                        self.source_image,
                        destination_morph_lines,
                        source_morph_lines,
                    ...
                    self.morphed_image = (
                        self.warped_source * (1.0 - self.param_t)
                        + self.warped_destination * self.param_t
                    )
                    self.morphed_image[..., -1] = 1.0

                    self.rescaled_warped_destination = cv2.resize(
                        self.warped_destination,
                        (self.rescaled_width, self.rescaled_height),
                        interpolation=cv2.INTER_AREA,
                    )

                    self.rescaled_morphed_image = cv2.resize(
                        self.morphed_image,
                        (self.rescaled_width, self.rescaled_height),
                        interpolation=cv2.INTER_AREA,
                    )
                ...
                self.rescaled_warped_source = cv2.resize(
                    self.warped_source,
                    (self.rescaled_width, self.rescaled_height),
                    interpolation=cv2.INTER_AREA,
                )
        ...
        if num_lines > 0 and self.show_warped_cursor and within_bound:
            ...
            if self.left_buffer == "S" and self.right_buffer == "D":
                ...
                if side == "left":
                    mouse_project = algos.multiple_line_pair_algorithm(
                        self.mouse_image_location,
                        ps_prime,
                        qs_prime,
                        ps,
                        qs,
            ...
            if not invalid_config:
                ...
                mouse_project = img_ops.unnormalize_coordinates(
                    mouse_project[None], *self.source_image.shape[:2]
                )
                self.mouse_project_int = mouse_project[0].astype(int)
                ...
                if not invalid_config:
                    self.add_raster_point(
                        mouse_project,
                        palette[(self.source_morph_index // 2) % num_colors],
                        point_size=4,
                    )
        ...
        if within_bound and side == "left":
            ...
            self.add_raster_point(
                self.mouse_location,
                palette[(self.destination_morph_index // 2) % num_colors],
                point_size=4,
            )

        if self.show_source_morph_lines:
            self.add_morph_lines(
                self.source_morph_lines, self.source_morph_index, palette, False
            )
    ...
    def on_key_press(self, symbol: int, modifiers: Any):
    ...
    def export(self):
        ...
        if not self.args.output_path:
            ...
            h, w = self.app_resolution
            num_lines = min(
                self.source_morph_index // 2, self.destination_morph_index // 2
            )
            if num_lines > 0:
                source_image_name = os.path.splitext(
                    os.path.basename(self.args.source_image_path)
                )[0]
                if self.args.destination_image_path:
                    destination_image_name = os.path.splitext(
                        os.path.basename(self.args.destination_image_path)
                    )[0]
                ...
                source_morph_lines = self.canvas_to_image(
                    source_morph_lines, "left"
                ).astype(int)
                destination_morph_lines = self.canvas_to_image(
                    destination_morph_lines, "right"
                ).astype(int)
                csv_ops.write_csv(
                    source_morph_lines.reshape(-1, 4),
                    os.path.join(
                        self.args.output_path, f"{source_image_name}_source.csv"
                    ),
                )
                csv_ops.write_csv(
                    destination_morph_lines.reshape(-1, 4),
                    os.path.join(
                        self.args.output_path, f"{source_image_name}_destination.csv"
                    ),
                )
                img_ops.write_image(
                    np.flip(self.warped_source, 0),
                    os.path.join(
                        self.args.output_path, f"{source_image_name}_warpedsource.png"
                    ),
                )

                if self.args.destination_image_path:
                    img_ops.write_image(
                        np.flip(self.warped_destination, 0),
                        os.path.join(
                            self.args.output_path,
                            f"{source_image_name}_warpeddestination.png",
                        ),
                    )
                    img_ops.write_image(
                        np.flip(self.morphed_image, 0),
                        os.path.join(
                            self.args.output_path, f"{source_image_name}_morphed.png"
                        ),
                    )

                print(
                    f"Exported image and csv files to {os.path.abspath(self.args.output_path)}"
                )
            else:
    ...
    def export_video(self):
        ...
        if not self.args.output_path:
            ...
            if num_lines > 0:
                ...
                frame_ts = np.arange(0, self.num_frames + 1).astype(np.float32) / (
                    self.num_frames
                )
                ...
                for i, t in enumerate(list(frame_ts)):
                    ...
                    warped_source = algos.run_a2_algo(
                        self.source_image,
                        self.destination_image,
                        source_morph_lines,
                        destination_morph_lines,
                    ...
                    warped_source[..., -1] = 1.0
                    img_ops.write_image(
                        np.flip(warped_source, 0),
                        os.path.join(
                            self.args.output_path,
                            f"{source_image_name}_warpedsource_{i:06d}.png",
                        ),
                    )

                    if self.args.destination_image_path:
                        warped_destination = algos.run_a2_algo(
                            self.destination_image,
                            self.source_image,
                            destination_morph_lines,
                            source_morph_lines,
                        ...
                        morphed_image = (
                            warped_source * (1.0 - t) + warped_destination * t
                        )
                        morphed_image[..., -1] = 1.0
                        img_ops.write_image(
                            np.flip(warped_destination, 0),
                            os.path.join(
                                self.args.output_path,
                                f"{source_image_name}_warpeddestination_{i:06d}.png",
                            ),
                        )
                        img_ops.write_image(
                            np.flip(morphed_image, 0),
                            os.path.join(
                                self.args.output_path,
                                f"{source_image_name}_morphed_{i:06d}.png",
                            ),
                        )

                print(f"Algorithm success. Wrote frames to {self.args.output_path}.")

    def on_key_release(self, symbol: int, modifiers: Any):
    ...
    def on_mouse_drag(self, x, y, dx, dy, button):
        ...
        if self.select_mode:
            if (
                self.mouse_within_bound
                and self.mouse_side == "right"
                and self.focus_pane == "right"
            ):
                self.destination_morph_lines[
                    self.selected_idx // 2, self.selected_idx % 2
                ] = np.array(self.mouse_location)
            elif (
                self.mouse_within_bound
                and self.mouse_side == "left"
                and self.focus_pane == "left"
            ):
    ...
    def update_mouse_location(self, x, y):
        ...
        self.mouse_within_bound, self.mouse_side = self.check_image_bounds(
            self.mouse_location_int
        )
        if self.mouse_within_bound:
            self.mouse_image_location_int = self.canvas_to_image(
                np.array([self.mouse_location_int]), self.mouse_side
            )[0]
            self.mouse_image_location = img_ops.normalize_coordinates(
                self.mouse_image_location_int, *self.source_image.shape[:2]
            )
    ...
    def on_mouse_press(self, x, y, button):
        ...
        if self.verbose:
            ...
            print(
                f"        float : [{self.mouse_location[0]:.2f}, {self.mouse_location[1]:.2f}]"
            )
            if self.mouse_within_bound:
                print(f"    Click location (on image)")
                print(f"        int   : {self.mouse_image_location_int}")
                print(
                    f"        float : [{self.mouse_image_location[0]:.2f}, {self.mouse_image_location[1]:.2f}]"
                )
                if not self.invalid_config and self.show_warped_cursor:
                    ...
                    print(
                        f"        float : [{self.mouse_project[0]:.2f}, {self.mouse_project[1]:.2f}]"
                    )
        ...
        if not self.edit_arrows:
            if (
                ...
                if self.source_morph_index % 2 == 0:
                    self.source_morph_lines = np.concatenate(
                        [
                            self.source_morph_lines,
                            np.array([[self.mouse_location, [0, 0]]]),
                        ],
                        axis=0,
                    )
                    self.destination_morph_lines = np.concatenate(
                        [
                            self.destination_morph_lines,
                            np.array(
                                [
                    ...
                    self.destination_morph_lines[-1, 1] = np.array(
                        [self.mouse_location[0] + 1.0, self.mouse_location[1]]
                    )
                    self.focus_pane = "none"
            ...
            if (
                ...
                if self.destination_morph_index % 2 == 0:
                    ...
                    self.source_morph_lines[-1, 1] = np.array(
                        [self.mouse_location[0] - 1.0, self.mouse_location[1]]
                    )
                    self.focus_pane = "none"
            ...
            if self.mouse_side == "left":
                if self.source_morph_index >= 2:
                    disps = (
                        self.source_morph_lines - np.array(self.mouse_location)
                    ).reshape(-1, 2)
                    dists = (disps**2).sum(-1)
                ...
                if self.destination_morph_index >= 2:
                    disps = (
                        self.destination_morph_lines - np.array(self.mouse_location)
                    ).reshape(-1, 2)
                    dists = (disps**2).sum(-1)
                    if np.min(dists) < 0.025:
                        self.selected_idx = np.argmin(dists)
...
if __name__ == "__main__":
    ...
    app_group.add_argument(
        "--destination-line-path",
        type=str,
        help="Path to csv file to import preset right morph lines.",
    )
    app_group.add_argument(
        "--supersampling", type=int, default=1, help="The patch size for supersampling."
    )
    app_group.add_argument(
        "--vectorize", action="store_true", help="Set this to enable vectorization."
    )
    app_group.add_argument(
        "--reference-solution",
        action="store_true",
