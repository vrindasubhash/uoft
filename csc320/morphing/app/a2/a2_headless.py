...
if __name__ == "__main__":
    ...
    if not args.destination_image_path:
        ...
        if (
            destination_image.shape[0] != source_image.shape[0]
            or destination_image.shape[1] != destination_image.shape[1]
        ):
            print(
                "The source image dimensions should match the destination image dimensions."
                f" Got {source_image.shape} and {destination_image.shape}."
            )
            sys.exit(0)
        print(
            f"Running algorithm on {os.path.abspath(args.source_image_path)} "
    ...
    source_morph_lines = img_ops.normalize_coordinates(
        source_morph_lines, *source_image.shape[:2]
    )

    destination_morph_lines = csv_ops.load_csv(args.destination_line_path).reshape(
        -1, 2, 2
    )
    destination_morph_lines = img_ops.normalize_coordinates(
        destination_morph_lines, *source_image.shape[:2]
    )
    ...
    if args.destination_image_path:
        destination_image_name = os.path.splitext(
            os.path.basename(args.destination_image_path)
        )[0]
    ...
    if args.num_frames == 1:
        ...
        frame_ts = (
            np.arange(0, args.num_frames + 1).astype(np.float32) / args.num_frames
        )
    ...
    for i, t in enumerate(list(frame_ts)):
        ...
        img_ops.write_image(
            ...
            os.path.join(
                args.output_path, f"{source_image_name}_warpedsource_{i:06d}.png"
            ),
        )
        if args.destination_image_path:
            ...
            img_ops.write_image(
                ...
                os.path.join(
                    args.output_path, f"{source_image_name}_morphed_{i:06d}.png"
                ),
            )
