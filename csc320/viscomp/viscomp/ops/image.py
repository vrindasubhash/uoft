...
def read_image(path: str) -> npt.NDArray[np.float32]:
...
def write_image(image: npt.NDArray[np.float32], path: str) -> None:
    """Loads an image as a Numpy array.
...
def create_coordinates(h: int, w: int) -> npt.NDArray[np.float32]:
    ...
    """
    window_x = np.arange(w, dtype=np.float32)
    window_y = np.arange(h, dtype=np.float32)
    coords = np.stack(np.meshgrid(window_x, window_y), axis=-1).astype(np.float32)
    return coords


# def normalize_coordinates(
#     coords: npt.NDArray[np.float32], h: int, w: int
