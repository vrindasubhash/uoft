...
def load_csv_path(path: str) -> str:
...
if __name__ == "__main__":
    ...
    source_coords = source_coords  # img_ops.normalize_coordinates(source_coords, h, w)
    destination_coords = (
    ...
    points = (
        homography @ np.concatenate([destination_coords, np.ones([4, 1])], axis=-1).T
    )
    ...
    output_path = os.path.abspath(
        os.path.join(args.output_path, image_name + "_output.png")
    )
