...
class Assignment1App(base_app.BaseAssignmentApp):
    ...
    def init_state(self) -> None:
        ...
        self.left_corners = np.zeros([4, 2], dtype=np.float32)
        self.left_corner_index = 0
        self.right_corners = np.zeros([4, 2], dtype=np.float32)
        self.right_corner_index = 0
        self.backprojected_corners = np.zeros([4, 2], dtype=np.float32)
        ...
        if input_ratio < screen_ratio:
            ...
            self.buffer[offset : offset + self.input_image.shape[0], 0 : (w // 2)] = (
                np.flip(self.input_image, 0)
            )
        else:
            ratio = self.input_image.shape[0] / h
            resized_width = int(self.input_image.shape[1] * (1.0 / ratio))
            self.input_image = cv2.resize(
                self.input_image, (resized_width, h), interpolation=cv2.INTER_AREA
            )
            offset = ((w // 2) - self.input_image.shape[1]) // 2
            self.buffer[0:h, offset : offset + self.input_image.shape[1]] = np.flip(
                self.input_image, 0
            )
    ...
    def draw_imgui(self) -> None:
        ...
        expanded, visible = imgui.collapsing_header(
            "Debug", True, imgui.TREE_NODE_DEFAULT_OPEN
        )
        if expanded:

            imgui.text(
                f"Mouse Location: {self.mouse_location[0]:.02f}, {self.mouse_location[1]:.02f} "
    ...
    def add_corners(
        ...
        for i in range(max(corner_index - 1, 0)):
            self.add_raster_line(
                corners[i], corners[(i + 1) % 4], [0, 0, 1, 1], line_width=4
            )
        if corner_index == 4:
    ...
    def render(self) -> None:
        ...
        if self.dirty_canvas:
            if self.left_corner_index == 4 and self.right_corner_index == 4:
                ...
                # convert from image [0, 1] coordinates back to [w, h] coordinates
                image_source_coords = ((self.left_corners + 1.0) / 2.0) * np.array(
                    self.buffer.shape[1::-1]
                ).reshape(1, 2)
                image_dest_coords = ((self.right_corners + 1.0) / 2.0) * np.array(
                    self.buffer.shape[1::-1]
                ).reshape(1, 2)

                output = algos.run_a1_algo(
                    self.buffer, self.buffer, image_source_coords, image_dest_coords
                )
    ...
    def get_current_state_text(self) -> list[str]:
        ...
        for i in range(4):
            if i < self.left_corner_index:
                x = str(
                    int(
                        ((self.left_corners[i, 0] + 1.0) / 2.0)
                        * self.screen["tex"].shape[1]
                    )
                )
                y = str(
                    int(
                        ((self.left_corners[i, 1] + 1.0) / 2.0)
                        * self.screen["tex"].shape[0]
                    )
                )
            else:
                x = "null"
                y = "null"
            text = f"{colors[i]}: [{x.rjust(5)} {y.rjust(5)}]"

            if i < self.right_corner_index:
                x = str(
                    int(
                        ((self.right_corners[i, 0] + 1.0) / 2.0)
                        * self.screen["tex"].shape[1]
                    )
                )
                y = str(
                    int(
                        ((self.right_corners[i, 1] + 1.0) / 2.0)
                        * self.screen["tex"].shape[0]
                    )
                )
            else:
            ...
            if self.right_corner_index == 4 and self.left_corner_index == 4:
                x = str(
                    int(
                        ((self.backprojected_corners[i, 0] + 1.0) / 2.0)
                        * self.screen["tex"].shape[1]
                    )
                )
                y = str(
                    int(
                        ((self.backprojected_corners[i, 1] + 1.0) / 2.0)
                        * self.screen["tex"].shape[0]
                    )
                )
                text += f"  [{x.rjust(5)} {y.rjust(5)}]"
    ...
    def on_mouse_press(self, x: Any, y: Any, button: Any) -> None:
        ...
        if scaled_x < 0.0:
            ...
            if self.right_corner_index < 4:
                ...
                self.right_corners[self.right_corner_index, 1] = scaled_y
        if self.left_corner_index == 4 and self.right_corner_index == 4:
            self.H_matrix = algos.calculate_homography(
                self.left_corners, self.right_corners
            )
            points = (
                self.H_matrix
                @ np.concatenate([self.right_corners, np.ones([4, 1])], axis=-1).T
            )
