...
try:
    ...
    print(
        f"WARNING: Failed to import {backend_api}. You might be using an old version of glumpy. "
...
class BaseAssignmentApp(sys.modules[backend].Window, ABC):
    ...
    def __init__(self, args: Any):
        ...
        super().__init__(
            width=self.app_resolution[1],
            height=self.app_resolution[0],
            fullscreen=False,
            config=app.configuration.get_default(),
        )
        ...
        self.screen = gloo.Program(
            screen_vertex, screen_fragment, count=4, version="120"
        )
    ...
    def clear_raster_primitives(self) -> None:
        """Removes all instantiated raster primitives."""
        self.raster_line_buffers: list[Any] = []
        self.raster_point_buffers: list[Any] = []
    ...
    def add_raster_lines(
        self,
        origins: npt.NDArray[np.float32],
        destinations: npt.NDArray[np.float32],
    ...
    def add_raster_point(
        self,
        origin: npt.NDArray[np.float32],
        color: npt.NDArray[np.float32],
        point_size: Optional[float] = 2,
    ) -> None:
        vertex_buffer = np.zeros(
