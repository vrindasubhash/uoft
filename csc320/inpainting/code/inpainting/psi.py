...
if args.reference_solution:
    from binaries.compute import computeC, computeGradient, computeNormal
else:
    from inpainting.compute import computeC, computeGradient, computeNormal
...
class PSI:
    ...
    def pixels(self, returnValid=None):
        if self._img is not None:
            ...
            pix, valid = getWindow(self._img, self._coords, self._w)
        else:
            pix = None
        if len(pix.shape) == 2:
            pix = pix[:,:,None]
    ...
    def filled(self):
        if self._fld is not None:
            ...
            fill, _ =  getWindow(self._fld, self._coords,
