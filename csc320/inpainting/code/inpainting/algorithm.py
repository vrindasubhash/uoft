...
class Inpainting:
    ...
    def readImage(self, fileName, key):
        ...
        elif key in self._images:
            # assign the image to the specified key in the dictionary
            self._images[key] = image
            success = True
            msg = f"Image loaded successfully for key '{key}'"
        else:
            msg = f"Invalid key '{key}' provided"
        #########################################
    ...
    def writeImage(self, fileName, key):
        ...
        #########################################
        # check if the image for the specified key exists
        if key in self._images and self._images[key] is not None:
            # try to save the image
            if cv.imwrite(fileName, self._images[key]):
                success = True
                msg = f"Image saved successfully to {fileName}"
            else:
                msg = f"Failed to save image to {fileName}"
        else:
            msg = f"No image data found for key '{key}'"
        #########################################
