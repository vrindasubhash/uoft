...
class ImageViewer(Scatter):
    ...
    def display_opencv_image(self, im=None, name=None):
        if im is not None:
            ...
            self._openCVRowColToKivyXY = np.array(
                [[0, 1, 0], [-1, 0, im.shape[0] - 1], [0, 0, 1]]
            )
        else:
    ...
    def on_touch_down_callback(self, touch):
        ...
        if touch.is_double_tap:
            ...
            with self.canvas:
                ...
                self.ud["lines"] = [
                    Line(
                        points=[touch.pos[0], 0, touch.pos[0], self.ids.image.height],
                        group=self.ud["group"],
                    ),
                    Line(
                        points=[
    ...
    def draw_rectangle_centered(self, r=0, c=0, radius=0, group="default"):
        ...
        self.draw_set_group(group)
        r = Line(
            rectangle=(x - radius, y - radius, 2 * radius + 1, 2 * radius + 1),
            group=group,
        )
    ...
    # function to handle a mouse button release event
    def on_touch_up_callback(self, touch):
        ...
        if touch.button != "left":
            return
        # get the dictionary where we store all the data to be displayed
        self.ud = touch.ud
        # make sure we've got the correct mouse event, in which we've
        # stored the data to be displayed
        if "group" in self.ud:
            # remove all the drawing instructions so that the line is erased
            # at the next window refresh (refresh happens automatically)
            self.canvas.remove_group(self.ud["group"])
        if "label" in self.ud:
            # remove all the text display widget
            # at the next window refresh (refresh happens automatically)
