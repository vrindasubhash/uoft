...
## You do NOT need to look at this file for your A3 implementation
...
class RootWidget(FloatLayout):
    ...
    def step_algorithm(self):
        # the algorithm is executed by calling the method of
        # the InpaintingControl class
        ok, msg = self.inpaintingControl.runAlgorithm(
            self.ids.imviewer, maxIterations=1
        )
        if not ok:
            # if the method returned an error, display a popup
            # with an error message
    ...
    def display_current_image(self):
        ...
        self.ids.imviewer.display_opencv_image(
            im=currentOpenCVImage, name=currentOpenCVImageName
        )
    ...
    def show_error_popup(self, title, message):
        try:
            ...
            self._popup = Popup(title=title, content=content, size_hint=(0.9, None))
            self._popup.open()
        except Exception as e:
    ...
    def show_debug(self):
        content = DebugDialog(
            ...
            show_intensities_callback=self.inpaintingControl.debug.setShowIntensities,
    ...
    def save(self, path, filename):
        ...
        if filename is not None:
            self._loadsave(os.path.join(path, filename), self.inpaintingControl.save, s)
