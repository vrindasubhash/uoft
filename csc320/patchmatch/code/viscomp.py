...
def parse_arguments(argv: List[str], prog: str = ""):
    ...
    Returns:
        Tuple containing:
            (1) True if all the required arguments are specified and False otherwise.
            (2) A Namespace() object containing the user-specified arguments and any optional arguments that take default values.
            (3) A list of any unrecognized/unprocessed command-line arguments.
            (4) An error message (if success=False).
    """

    # Initialize the command-line parser.
    parser = argparse.ArgumentParser(prog, description="Script for patch match.")

    # Check for reference-solution flag
    parser.add_argument(
        "--reference-solution",
        action="store_true",
    ...
    parser.add_argument(
        "--source", type=str, help="Path to source image", required=True
    )

    parser.add_argument("--target", type=str, help="Path to targe image", required=True)
    ...
    parser.add_argument(
        "--iters", type=int, help="Number of iterations", default=4, required=False
    )

    # Patch size.
    parser.add_argument(
        "--patch-size", type=int, help="Patch window size", default=7, required=False
    )
...
def main(argv: List[str], prog: str = ""):
    ...
    patmat.set_reference_solution(args.reference_solution)
    ...
    # Propagation and random search are enabled by default so we toggle them to disable.
